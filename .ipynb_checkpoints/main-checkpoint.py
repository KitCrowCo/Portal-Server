...
def load_server_router(path, item, prefix, dependencies={}, router_type="module", path_modifier="", verbose=VERBOSE):
    ...
    try:
        ...
        app.include_router(mod.router, prefix=f"/{prefix}/{item}", tags=[f"{item}:{router_type}"], dependencies=auth_dep)
        if verbose: print(f"Loaded {router_type} router: {path_modifier}{dash}{item}")
        metas[router_type].update({item: module_meta})
        if verbose: print(f" MODULE_META for {item}: {module_meta}")
        deps_with_meta = {**dependencies, "meta": module_meta, "refresh_system": dependencies.get("refresh_system")(f"/{prefix}/{item}", mod.router)} if dependencies else {"meta": module_meta}
        ...
        print(f"Error loading {item} router: {e}")
        return False
...
async def handle_login(response: Response, request: Request, username: str = Form(...), password: str = Form(...), next: str = Form(default="/"), db=Depends(get_db)):
    ...
    if user and verify_password(password, user.password_hash):
        ...
        return {"status": "ok"}
    return HTMLResponse(content="Invalid Username and Password", status_code=401)
...
async def dashboard(request: Request, user=Depends(get_current_user)):
    ...
    return templates.TemplateResponse(name = "base.html", request = request, context = {"request": request, "user": user, "PWA": True, "content": content_area, "code_mirror": True, **_portal_im.template_context(),
                        ...
                        "toolbars": {"top": UI.toolbar(side="top", content = top_content, size = "4.25rem", overlay = False, start_open = True, id = "portal-top-bar", nesting_level = 0)}})
