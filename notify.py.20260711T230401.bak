...
async def send_push(username: str, title: str, body: str, db: Session, url: str = "/") -> int:
    """
    Send push to all subscriptions for username. Prunes dead endpoints.
    Also delivers via WebSocket if the user is currently connected.
    Returns count of successful pushes.
    """
    # WS delivery — instant if user is online, no-op otherwise
    try:
        await _ws_manager.send_personal_message({"type": "notification", "title": title, "body": body, "url": url}, username)
    except Exception as e:
        logger.debug("WS notify error for %s: %s", username, e)
...
async def broadcast_push(title: str, body: str, db: Session, url: str = "/", role_filter: Optional[str] = None) -> int:
    """ Send to all subscriptions, optionally filtered by role. Joins PushSubscription → User for role filtering. """
    try:
        await _ws_manager.broadcast({"type": "notification", "title": title, "body": body, "url": url})
    except Exception as e:
