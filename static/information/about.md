...
- **Simple pieces, composed, are how complex and capable things get built — not a tradeoff against capability.** A small number of well-defined primitives (state scopes, a nesting shell system, a handful of UI component functions) working together is what produces a system that can do a lot, without each new feature requiring its own bespoke, fragile mechanism. Complexity concentrated in a few well-understood places is easier to reason about, extend, and fix than complexity spread thin across many ad hoc special cases — the same pattern you'd find underneath most things that look complicated from the outside but are actually a small set of rules applied consistently.
